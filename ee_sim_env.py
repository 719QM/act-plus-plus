...
class RMpaperEETask(base.Task):
    ...
    def before_step(self, action, physics):
        ...
        # action得分为两种情况：1.ee空间 mocap的位姿 2.关节空间 14个关节角度
        # action的第一位设置为模式位，即如果第一位为0,即为末端跟踪mocap；如果第一位为1,即为关节空间
        if action[0] == 0:
            a_len = (len(action) - 1) // 2
            action_left = action[1:a_len+1]
            action_right = action[a_len+1:]
        ...
        if action[0] == 1:
            ...
            arm_left = action_left[:6]
            arm_right = action_right[:6]
            gripper_left = action_left[6]
            gripper_right = action_right[6]
            unnormalized_gripper_left = RM_GRIPPER_UNNORMALIZE(gripper_left)
            unnormalized_gripper_right = RM_GRIPPER_UNNORMALIZE(gripper_right)

            all_actions = np.concatenate(
                [np.array([unnormalized_gripper_left, unnormalized_gripper_right]), arm_left, arm_right])

            np.copyto(physics.data.ctrl[:14], all_actions)
        # vx300s_left/left_finger,  vx300s_left/right_finger, vx300s_right/left_finger, vx300s_right/right_finger
