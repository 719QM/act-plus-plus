...
def main(args):
    ...
    为在仿真环境中生成演示数据，可以按照以下步骤进行。
    这些步骤包括运行策略（ee space）、获取关节轨迹、替换抓手关节位置、在仿真环境(joint space)中重放关节轨迹并记录观察结果，
    最后保存数据并继续下一集数据收集。
    """
    ...
    for episode_idx in range(num_episodes):
        ...
        # HDF5
        t0 = time.time()
        dataset_path = os.path.join(dataset_dir, f'episode_{episode_idx+40}')
